...
def render(data: dict[str, Any]) -> str:
    ...
    return f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>{html(meta["company"])} · A股公司研究仪表盘</title><style>{STYLE}</style></head><body><main><header class="masthead"><div><p class="eyebrow">A-SHARE / COMPANY RESEARCH</p><h1>{html(meta["company"])}</h1></div><p class="asof">{html(meta["ticker"])} · {html(meta["exchange"])}<br>截止 {html(meta["as_of"])}<br>事实 {fact_count} · 推断 {sum(x["kind"] == "推断" for x in data["evidence"])} · 判断 {sum(x["kind"] == "判断" for x in data["evidence"])}</p></header><div class="badges">{badge("事实")} 已核验数据 · {badge("推断")} 机制解释 · {badge("判断")} 研究标签 {html(summary["research_label"])} · <span class="badge badge-inference">{html(meta["mode"])} · 置信度 {html(meta["confidence"])}</span></div><section class="panel thesis"><h2>当前研究命题：<strong>{html(dashboard["headline_thesis"]["statement"])}</strong></h2><p>{html(dashboard["headline_thesis"]["detail"])}</p></section><section class="signal-grid" aria-label="五个关键研究信号">{signal_html}</section><section class="grid"><article class="panel market"><span class="num">01 · MARKET PRICING</span><h2>市场定价什么</h2><p>{html(gap["market_pricing"])}</p></article><article class="panel facts"><span class="num">02 · VERIFIED FACTS</span><h2>已验证事实</h2><p>{html(gap["verified_facts"])}</p></article><article class="panel proof"><span class="num">03 · NEXT PROOF</span><h2>下一步验证</h2><p>{html(gap["next_proof"])}</p></article></section><section class="split"><article class="panel"><span class="num">CATALYSTS</span><h2>催化剂</h2><ul class="timeline">{timeline(dashboard["catalysts"])}</ul></article><article class="panel"><span class="num">INVALIDATION</span><h2>证伪信号</h2><ul class="timeline">{timeline(dashboard["invalidations"])}</ul></article></section><details class="details"><summary>展开关键证据台账 · 只在需要追溯时阅读</summary><section class="panel table-wrap"><table><thead><tr><th>信号</th><th>内容</th><th>类型</th><th>来源与时间</th></tr></thead><tbody>{"".join(evidence_rows) or '<tr><td colspan="4">未取得可验证证据。</td></tr>'}</tbody></table></section></details><details class="details"><summary>展开业务、估值、风险与技术面</summary><section class="grid"><article class="panel"><h2>业务与利润引擎</h2><ul class="timeline">{engine}</ul></article><article class="panel"><h2>催化剂日历</h2><ul class="timeline">{catalysts}</ul></article><article class="panel"><h2>估值与情景</h2><p>{html(data["valuation"]["method"])}：{html(data["valuation"]["conclusion"])}</p><div class="table-wrap"><table><thead><tr><th>情景</th><th>假设</th><th>触发</th></tr></thead><tbody>{scenarios}</tbody></table></div></article><article class="panel"><h2>风险与证伪</h2><ul class="timeline">{risks}</ul></article></section><section class="panel" style="margin-top:10px"><h2>技术面</h2><p>{html(technical_text)}</p><div class="notice"><strong>待核验：</strong><ul>{gaps}</ul></div></section></details><details class="details"><summary>展开来源与完整研究记录</summary><section class="panel source table-wrap"><h2>信息来源</h2><table><thead><tr><th>来源</th><th>URL</th></tr></thead><tbody>{source_rows}</tbody></table></section></details><footer>首层只呈现可决策信息；完整事实、推断、判断和来源均可展开追溯，不构成个性化投资建议。</footer></main></body></html>'''
