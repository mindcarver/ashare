...
description: 生成可审计的A股每日深度复盘，覆盖指数与宽度、成交与量能、涨跌停与情绪周期、封板结构、多周期资金、行业/概念/主题资金、资本共现与集中度、产业催化链、龙虎榜结构、前一涨停池及可自动结算验证点。当用户要复盘A股、识别主线与伪板块、分析资金去了哪里、检查科技是否放量启动或比较盘后报告时使用；不输出买卖、仓位或目标价。
---

# A股每日盘面复盘

本技能描述一个交易日的市场结构。`1.2` 把**双确认主线矩阵**与**延续性检验**做成可复算结构；`1.3` 再加入深度分析层，把量能/均线、情绪周期、封板时序、多周期资金、产业催化和龙虎榜纳入同一审计契约，并增加集中度、重叠、反方证据与验证语义绑定。
...
需要做比普通盘面更深的复盘时，再读取[深度分析层契约](references/deep-analysis.md)，在顶层声明可选`deep_analysis`：个股封板与1/3/5/10日资金、量能/均线/120日分位、情绪周期、资金共现与伪板块、产业催化证据链、龙虎榜结构。它不进入十章覆盖计数；未声明时不产生深度输出。
...
生成器可直接读取旧`schema_version=1.0/1.1/1.2`产物并保守归一化。1.2验证点若不能从标题证明与condition的全市场语义一致，会降为“定性观察点”；不会为了兼容而伪造`subject`或深度证据。新输入必须直接使用1.3。
...
一键取数：`make fetch-market DATE=<交易日> [OUT=market.json] [CTX=ctx.json]` 用东财一手数据组装 1.3 输入并自动跑一次契约校验；主线、延续性、事件和深度分析由`--context`显式声明，缺省不猜。详见避坑清单第十一节。
