...
description: 生成可审计的A股每日盘面复盘，覆盖主要指数、成交活跃度、上涨/下跌家数、涨跌停、短线情绪观察与多条件阈值体检、板块表现与板块资金流、资金证据、风格结构、双确认主线矩阵、前一涨停池延续性检验、公开事件和后续验证点。当用户说“复盘今天A股”“今天盘面怎么样”“每日收盘复盘”“看市场宽度、连板或短线情绪”“哪些主题是双确认主线”“昨天涨停的今天怎么样”时使用；不输出买卖或仓位指令。
---

# A股每日盘面复盘

本技能描述一个交易日的市场结构：指数怎么走、多少股票参与、成交是否扩张、短线情绪有哪些可见特征、板块和风格如何分化、资金证据是什么、出现了哪些显式背离。在此之上，`1.2` 版本把两类跨日/跨维度的判断也做成了可复算的结构：**双确认主线矩阵**（游资情绪面×机构资金面）与**延续性检验**（前一涨停池当日表现与晋级率）。
...
生成器可直接读取旧`schema_version=1.0`真实产物并保守归一化：用原始输入SHA作为证据包SHA，根据最晚`fetched_at`建立快照，把可识别的窗口和资金方法显式化；无法证明股票池一致的情绪降为partial，无法机器计算的旧验证点保留为“定性观察点”而不自动判定。`1.1`输入沿同一条升级链升到`1.2`：只改版本号并写入迁移警告，不回填任何新章节，报告与摘要保持逐字节不变。新输入必须直接使用1.2。
...
7. 先结算上一期验证点，再列出公开事件和结构化未来验证点。
8. 最后只写显式结构信号、输入声明的反方证据、限制和需要后续验证的条件。
...
一键取数：`make fetch-market DATE=<交易日> [OUT=market.json] [CTX=ctx.json]` 用东财一手数据组装 1.2 输入并自动跑一次契约校验；需人工判断的主线归组 / 延续性检验 / 事件由 `--context` 显式声明，缺省即按 unknown 处理。详见避坑清单第十一节。
