...
- `analysis_mode=core`：基础十章交付。允许附带部分深度组件，但报告必须明确“深度模式未启用”。
- `analysis_mode=deep`：深度交付承诺。六个`deep_analysis`组件必须全部声明；没有证据的组件使用`availability=unknown`并写原因，不能直接省略。

旧Schema 1.0–1.3统一升级为`core`，保留原证据，不推断用户当时承诺了深度交付。

## 四轴语义

| 轴 | 数据 | 支持条件 | 冲突条件 |
|---|---|---|---|
| 纵｜时间演化 | 情绪周期窗口、窗口新低、前涨停池晋级健康线 | 未触及声明窗口新低且晋级率不低于声明健康线 | 触及新低或低于健康线 |
...
| 验｜事后闭环 | 未来验证点、已到期结果及scope | 至少存在一个结构化未来点或到期结果 | 无点时保持unknown，不当作失败 |
